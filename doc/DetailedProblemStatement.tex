...
9 & Normalize Image to appear straight. & Helpful\\\hline
10 & Make the report excel readable. & Helpful\\\hline
11 & Compare two spreads to determine differences. & Helpful\\\hline
\end{tabular}
\subsection{Form}
\paragraph{} The necessary specifications for analyzing the an image are detailed in the Supplementary Specifications section. These include analyzing an inputed image for bullet holes, being able to correct for image pitch, performing the statistical calculations on the locations of the holes, and allowing for extra test variables to be inputed.
\subsection{Economy}
...
\paragraph{}Any user that has an iOS device with a suitable camera and an operating system number that is compatible should be able to download the app from the App Store and use it to analyze target. The app should conform to Apple's Human Interface Guidelines and be easy for any user to pick up and use. It should follow standard user interface paradigms for an iOS app. 
\subsubsection{Development Organizational Constraints}
\paragraph{} Students from Rose-Hulman Institute of Technology will develop the app. Because we were given an acceptable image recognition algorithm we will first work to make sure it will run effectively on an iOS device. We will make any necessary changes to allow the algorithm to run properly. We will also have to re-imagine the user interface to make it work well on an iOS device. We will focus on iPhones and iPod touches working with an iPad only if we have time.
\subsection{Context}
